...
int nthreads;                           // making these global variables so they can be used in routine!
scene_t scene;
float scaled_color[width][height][3];   // changed to 3d array, explanation is in a comment below

void* routine(void * startPos)
{
    // gapsize is the amount each thread should differ from the previous one so that we can parallelize it
    int startingPosition = *(int*) startPos;
    ...
    /* for every pixel */
    for( int px=startingPosition; px<width; px+=nthreads)
    {
...
int main( int argc, char **argv )
{
    ...
    pthread_t threads[nthreads];        // make an array with the size of the number of threads that we went

    int startPos[nthreads]; // store the ints here for the start pos because otherwise there is a race condition!! (learned that the hard way =( )

    for (int i = 0; i < nthreads; i++){
        startPos[i] = i;
        int createThread = pthread_create(&threads[i], NULL, routine, (void*)(startPos + i));
        if (createThread)
            fprintf(stderr, "Could NOT create a new thread and an error was thrown");
    }

    // now that you have the threads doing their tasks ... if you dont join them here, then one thread might finish
    // everything and keep going. Thus you should join  them after the main tasks:

    for (int i = 0; i < nthreads; i++){
        pthread_join(threads[i], NULL);
    }

    // the printing part can't be done in parallel, since you want it to be done in a particular order
    // thus you do it AFTER the part where the computation took place, and also we need to change the 
    // implementation of scaled_color. Now each pixel width and height has three different values that
    // need to be tracked, so a 3D array [width][height][3] ... also make this global!
